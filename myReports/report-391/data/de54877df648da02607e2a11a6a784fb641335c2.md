...
```
Error: locator.innerText: Error: strict mode violation: locator('//h5[text()=\' Click Me (Inside frame)\']//following::iframe') resolved to 3 elements:
...
Call log:
  - waiting for locator('//h5[text()=\' Click Me (Inside frame)\']//following::iframe').contentFrame().locator('#Click')

    at C:\Users\DELL\Desktop\Photon_JS\tests\day07\frames.spec.ts:68:35
```
...
```yaml
- link:
  - /url: /dashboard.xhtml;jsessionid=node0v5twuj4zun9cxka4qcsenuoz1123363.node0
- link "":
  - /url: "#"
- link:
  - /url: "#"
...
```ts
  ...
  65 |
  66 |         const frameEle= page.frameLocator("//h5[text()=' Click Me (Inside frame)']//following::iframe")
  67 |         const clicEle=frameEle.locator("#Click")
> 68 |         console.log(await clicEle.innerText())
     |                                   ^ Error: locator.innerText: Error: strict mode violation: locator('//h5[text()=\' Click Me (Inside frame)\']//following::iframe') resolved to 3 elements:
  69 |         await clicEle.click()
  70 |         console.log(await clicEle.innerText())
  71 |
  72 |         
  73 |     //  await page.locator("//h5[text()=' Click Me (Inside Nested frame)']")
  74 |     //            .frameLocator("//iframe").frameLocator("#frame2")
  75 |     //            .locator("#Click").click()
  76 |
  77 |     const innerFrameEle= page.frameLocator("//h5[text()=' Click Me (Inside Nested frame)']//following::iframe")
  78 |               .frameLocator("#frame2")
  79 |             await innerFrameEle.locator("#Click").click()
  80 |             console.log(await innerFrameEle.locator("#Click").innerText())
  81 |             
  82 |             })
```